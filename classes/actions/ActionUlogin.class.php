...
class PluginUlogin_ActionUlogin extends ActionPlugin {
	...
	public function EventIndex() {
		...
		if(!$token)
		{
			$this->Message_AddErrorSingle($this->Lang_Get('plugin.ulogin.ulogin_token_error'));
		...
		try
		{
			...
			//Проверка наличия у полученных данных уникального пользовательского идентификатора
		...
		else
		{
			...
			if($this->oUserCurrent)
			{
				...
				if($this->bind($this->oUserCurrent, $user['identity']))
				{
					...
					$this->Message_AddNotice($this->Lang_Get('plugin.ulogin.ulogin_success_binded'));

					Router::Location(Config::Get('path.root.web').'/');
				}
				else
				{
					$this->Message_AddErrorSingle($this->Lang_Get('plugin.ulogin.ulogin_this_else_registered'));
					return Router::Action('error');
				}
			}
			else
	...
	{	
		...
		if ($user_data = $this->Session_Get('ulogin_user_data') and isPost('submit_register')) {
			...
			if (!func_check(getRequest('login'),'login',3,30)) {
				$this->Message_AddError($this->Lang_Get('plugin.ulogin.registration_login_error'),$this->Lang_Get('plugin.ulogin.error'));
				$bError=true;
			}
			//  Проверка мыла
			 
			if (!func_check(getRequest('mail'),'mail')) {
				$this->Message_AddError($this->Lang_Get('plugin.ulogin.registration_mail_error'),$this->Lang_Get('plugin.ulogin.error'));
				$bError=true;
			}
			...
			if (!func_check(getRequest('password'),'password',5)) {
				$this->Message_AddError($this->Lang_Get('plugin.ulogin.registration_password_error'),$this->Lang_Get('plugin.ulogin.error'));
				$bError=true;
			} elseif (getRequest('password')!=getRequest('password_confirm')) {
				$this->Message_AddError($this->Lang_Get('plugin.ulogin.registration_password_error_different'),$this->Lang_Get('plugin.ulogin.error'));
				$bError=true;
			}
			...
			if ($this->User_GetUserByLogin(getRequest('login'))) {
				$this->Message_AddError($this->Lang_Get('plugin.ulogin.registration_login_error_used'),$this->Lang_Get('plugin.ulogin.error'));
				$bError=true;
			}
			...
			if ($this->User_GetUserByMail(getRequest('mail'))) {
				$this->Message_AddError($this->Lang_Get('plugin.ulogin.registration_mail_error_used'),$this->Lang_Get('plugin.ulogin.error'));
				$bError=true;
			}
	...
	*/
	...
	/*
	 * Метод для привязки новых аккаунтов(если пользователь не авторизован изначально. доступно только с расширенной формой регистрации)
	 * 
	...
	private function UserRegistration($oUser, $uLoginUserData)
	{
		if ($oUser = $this->User_Add($oUser) and $this->bind($oUser, $uLoginUserData['identity'])) {
			...
			$this->Message_AddNotice($this->Lang_Get('plugin.ulogin.ulogin_success_register'));
			Router::Location(Config::Get('path.root.web').'/');	
			return true;			
		}
	...
	public function EventAdmin() {
		...
		if(isPost('admin_submit'))
		{
			...
			$this->Message_AddNotice($this->Lang_Get('plugin.ulogin.ulogin_settings_saved'));
		}
	...
	public function UploadAvatar($aFile,$oUser) {
		...
		if($sError=$oImage->get_last_error()) {
			...
			// $this->Message_AddError($sError,$this->Lang_Get('plugin.ulogin.error'));
			@unlink($sFileTmp);
			return false;
		}
